...
app.get("/scrape", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (articleContent?.textContent) res.json({ content: articleContent.textContent });
...
app.post("/get-recommendations", async (req, res) => {
  const { articles, title } = req.body;
  if (!articles || !title) return res.status(400).json({ error: "Missing inputs" });

  try {
    const { data } = await axios.post("http://localhost:5000/recommend", { articles, title });
    res.json(data);
  } catch {
...
app.post("/predict-fakeness", async (req, res) => {
  ...
  try {
    const { data } = await axios.post("http://localhost:5000/predict-fakeness", { text });
    res.json(data);
  } catch {
    res.status(500).json({ error: "Failed to predict fakeness" });
  }
